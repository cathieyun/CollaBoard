...
//    
	...
	private void drawOval(Oval oval) {
		...
		for (int i = 0; i < canvasModel.getDrawingObjectListUndoIndex(); i++) {
			...
			drawOrRedrawDrawingObject(currentDrawingObject, "redraw");
		}
	 ...
	 * the type.
	 * 
	 * If the Drawing Object is being drawn for the first time, we need to
	 * increment the undo index. As such, pass the string "draw" into the
	 * method.
	 * 
	 * @param d
	 *            the drawingObject to redraw onto the canvas
	 * @param drawOrRedraw
	 *            a string used to determine if the drawing Object is being
	 *            drawn for the first time "draw" means that we are drawing it
	 *            for the first time
	 */
	public void drawOrRedrawDrawingObject(DrawingObject d, String drawOrRedraw) {
		...
		if (drawOrRedraw.equals("draw")) {
			canvasModel.getAndIncrementIndex();
			System.out.println("Undo index was incremented in the drawOrRedraw method.");
		}
		this.repaint();
	}
