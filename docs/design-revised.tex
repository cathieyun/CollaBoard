...
entered a whiteboard. A user can only be on one Whiteboard at a time, and usernames are unique (no duplicates are allowed), and we only allow alphanumeric characters and no spaces for usernames.
...
\item Threadsafe through use of the monitor pattern
\end{itemize}
...
\begin{itemize}
	 \item \textbf {LIST} List of current, active whiteboard IDs.
     ...
     \item \textbf {NEWBOARD} Encodes the whiteboard ID of a whiteboard that has just been created to update the list of current whiteboard IDs on the whiteboard selection pane.
...
Test the server-client protocol and the client-server protocol.
\begin{itemize}
\item Client connect: make sure that a client can open a socket and connect to the server
\item MakeUser: make sure that a user can create a valid username
\item MakeUser: make sure that the server does not allow the user to create a duplicate username
\item MakeWhiteboard: make sure that the user can create a valid whiteboard.
\item MakeWhiteboard: make sure that the user cannot create a duplicate whiteboard.
\item NewBoard: make sure that other connected clients are notified if another user creates another whiteboard
\item Switchboard: make sure that the user can switch to a different whiteboard
\item Enter: make sure that the user can enter a valid whiteboard
\item Exit: make sure that the user can exit a whiteboard
\item List: make sure the server sends a list of current whiteboard IDs to the client when the client connects
\end{itemize}
...
\end{itemize}
Users entering the same whiteboard at the same time will see the correct list of users.
\begin{itemize}
\item Create whiteboard 1.
\item Two people each open a client connection.
\item Both people enter whiteboard 1 at the same time by selecting it from the list of existing whiteboards.
\item Expect them both to display the same (and correct) list of active users.
\end{itemize}
...
\item Two people should try drawing and undoing things at the same time. The state of their boards after the series of actions should be the same.
\end{itemize}
3 Users
\begin{itemize}
\item A person enters whiteboard 1 and draws something.
\item A second person enters whiteboard 1. It should be in the same state as person one sees the whiteboard.
\item A third person enters whiteboard 1. It should be in the same state as person one and two see the board.
\item The three people should draw on the whiteboard. The state of their boards after the series of actions should be the same.
\end{itemize}
\end{document}